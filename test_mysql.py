...
def get_mysql():
    try:
        ...
        # sql = """
        #   insert into test (id,name,arg)
        #   values ('999','0-000','99')
        # """

        sql = 'INSERT INTO {table}(id,name,arg) VALUES ({values})'.format(table="test", values=a)
        #sql = 'INSERT INTO test (id,name,arg) VALUES (%s);' % ','.join(a)
        # sql = 'INSERT INTO test (id,name,arg) VALUES (%s);' % ','.join(a)
        print(sql)
        # tuple(data.values()
...
def main():
    ...
    #     url = 'https://search.51job.com/list/000000,000000,0000,32,9,99,%25E5%25A4%25A7%25E6%2595%25B0%25E6%258D%25AE%25E5%25BC%2580%25E5%258F%2591%25E5%25B7%25A5%25E7%25A8%258B%25E5%25B8%2588,2,' + '{}.html'.format(
    #         page)
    #     print("正在爬取第{}页".format(page))
    #     html = get_html(url)
    #     url_lists = re.findall('<script type="text/javascript">.*?engine_search_result":(.*),.*?"jobid_count"', html,
    #                            re.S)
...
def parse_html(html):
    try:
        ...
        # 公司名称
        company_name = html_xpath.xpath(
            "//div[contains(@class, 'tHeader')]/div[@class='in']/div[@class='cn']/p[@class='cname']/a/@title")
        company_name = company_name[0]
        # print("company_name:{}".format(company_name))
        # 工作地点
        info = html_xpath.xpath(
            "//div[contains(@class, 'tHeader')]/div[@class='in']/div[@class='cn']/p[contains(@class, 'msg')]/@title")
...
def save_csv(items):
    for i in items:
        if i != None:
            ...
            industry = i[9]
            salary = i[10]
    # print(job_title, company_name, work_place, work_place, education, recruit_number, release_time, company_nature,
    #       company_size, industry, salary)
    # sql = """insert into book_share_db.wether_test(job_title, company_name, work_place, work_place, education, recruit_number, release_time,  company_nature, company_size, industry, salary))   values( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
    # cursor.execute(sql, (
    # job_title, company_name, work_place, work_year, education, recruit_number, release_time, company_nature,
    # company_size, industry, salary))
